...
if [[ ${wf} == 1 ]]; then
  ...
  cfy local init --install-plugins -p blueprint/realeScWF-inhost.yaml -i input.yaml
else
   if [[ -d ~/realeScWF-1host ]]; then
      echo "previous workflow execution exists and will be deleted"
      rm -r ~/realeScWF-1host
   fi
  cfy local init --install-plugins -p blueprint/realeScWF-1host.yaml -i input.yaml
fi
